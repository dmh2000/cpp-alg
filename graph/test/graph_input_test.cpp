...
TEST(GraphInputTest, GraphDataInput) {
	...
	EXPECT_EQ(gd->edges.size(), 13);
	for (auto i = 0; i < gd->edges.size(); i++) {
...
void checkV(int n, const std::vector<int>& g, const std::vector<int>& t) {
...
TEST(GraphInputTest, GraphCreation) {
	...
	for (auto v = 0; v < graph->v(); v++) {
		checkV(v, graph->adj(v), test_adj[v]);
	}
}